...
<ul class="nav nav-list nav-pills nav-stacked abn-tree">
  <li ng-repeat="row in tree_rows | filter:{visible:true} track by row.branch.uid" ng-animate="'abn-tree-animate'" ng-class="'level-' + {{ row.level }} + (row.branch.selected ? ' active':'') + ' ' + row.classes.join(' ')" class="abn-tree-row"><a><span ng-click="row.branch.expanded = !row.branch.expanded"><i ng-class="row.tree_icon" class="indented tree-icon"></i></span><span ng-click="user_clicks_branch(row.branch)"><span class="indented tree-label">{{ row.label }}</span></span></a></li>
